...
Use `betterAgent()` to create an app with a programmatic API (`run`, `stream`) and an HTTP handler. The handler is a standard `Request → Response` function, so you can mount it in any framework that accepts a `Request` object.
...
Use `app.run()` and `app.stream()` to call agents directly from your server code, without an HTTP layer.
...
Mount `app.handler` as a route in your framework. Keep the route path aligned with the `baseURL` you passed to `betterAgent()`.
...
For framework-specific examples, see the integrations pages.
