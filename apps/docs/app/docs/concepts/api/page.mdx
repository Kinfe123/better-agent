...
Streams agent execution events and returns a `result` promise for the final run result:
...
Loads the stored durable history for a conversation when persistence is configured:

```ts
const conversation = await app.loadConversation("assistant", {
  conversationId: "conv_123",
});
```
...
The app exposes a `.handler` property that is a standard `Request → Response` function. Mount it under the same path as `baseURL`:
...
Keep `baseURL` and the mounted route path aligned. If `baseURL` is `/api`, mount the handler under `/api`.
