...
Better Agent uses `BetterAgentError` as the standard error type across the framework. It carries a stable `code` for programmatic handling so you can react to specific failure modes without parsing error messages.
...
try {
  ...
  if (error instanceof BetterAgentError) {
    ...
    console.log(error.context);   // structured details
  }
}
```
...
When a human-in-the-loop approval times out, Better Agent emits a `TOOL_APPROVAL_UPDATED` event with `state: "expired"` and the run fails with a `TIMEOUT` error code. See [Human in the Loop](/docs/concepts/hil) for timeout configuration.
