...
Better Agent uses `BetterAgentError` as the standard error type across the framework. It carries a stable `code` for programmatic handling so you can branch on the error type without parsing strings.
...
When `error.retryable` is `true`, the operation may succeed if retried — for example, a rate limit or a transient network failure. When it's `false`, retrying will not help and you should surface the error to the user or log it for investigation.
