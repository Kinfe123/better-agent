...
Use `createMCPClient()` to connect, `listTools()` to discover available tools, and `convertMCPTools()` to turn them into Better Agent server tools:
...
Each MCP tool becomes a server tool that proxies execution back to the MCP server. The `prefix` option namespaces the tool names to avoid collisions.
