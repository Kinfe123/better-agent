...
An output schema has a `schema` and optional `name` and `strict` fields. The `schema` accepts any Standard Schema library (Zod, Valibot, ArkType) or a raw JSON Schema object.
...
| `strict` | Enable strict mode where supported. |
...
Use the `output` field on `run()` or `stream()` to override the agent's default schema for a single call:
...
The per-run `output` schema fully replaces the agent's default for that run, and the return type updates accordingly.
