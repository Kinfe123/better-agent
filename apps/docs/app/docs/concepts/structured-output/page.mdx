...
Add `outputSchema` to your agent definition. The `schema` field accepts Zod, Valibot, ArkType, or raw JSON Schema:
...
| `strict` | Enables strict mode where supported by the provider. |
...
```ts
const stream = app.stream("summary", {
