...
For streaming, access `structured` through the result promise after the stream completes.
...
The per-run schema fully replaces the agent's default for that run, and the return type updates accordingly.
