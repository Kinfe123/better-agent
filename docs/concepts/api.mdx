...
Use `app.run()` and `app.stream()` to call agents directly from your server code, with no HTTP layer.
...
Mount `app.handler` on any route that matches `baseURL`. The handler accepts a standard `Request` and returns a `Response`.

```ts
// Next.js App Router example
export { GET, POST } from "./server"; // re-export the handler
```

Keep `baseURL` and the mounted route aligned. If the app uses `baseURL: "/api"`, mount the handler under `/api`. See the integration guides for framework-specific mounting instructions.
