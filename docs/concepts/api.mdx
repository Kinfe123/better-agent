...
Mount `app.handler` in your framework of choice. The `baseURL` you passed to `betterAgent()` and the route path you mount on must match.

```ts
// Next.js App Router example
export { app.handler as GET, app.handler as POST };
```

For framework-specific mounting instructions, see the integrations pages.
