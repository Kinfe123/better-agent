...
Use `app.run()` and `app.stream()` to call agents directly from server code without an HTTP layer.
...
Loads the stored durable history for a conversation. Requires a [Persistence](/docs/concepts/persistence) store to be configured.

## HTTP handler

The app exposes a `handler` property that is a standard `Request → Response` function. Mount it in your framework of choice. Keep `baseURL` and the mounted route aligned — if the app uses `baseURL: "/api"`, mount it under `/api`.

For framework-specific mounting instructions, see the integration guides for Next.js, SvelteKit, TanStack Start, and others.
