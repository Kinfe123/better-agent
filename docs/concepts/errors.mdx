...
Better Agent uses `BetterAgentError` as the standard error type across the framework. It carries a stable `code` for programmatic handling so you can branch on error type reliably across versions.
...
try {
  ...
  if (error instanceof BetterAgentError) {
    ...
    console.log(error.context);   // additional structured data
  }
}
```
...
| `context` | `Record<string, unknown>` | Additional structured data about the error. |
...
When `error.retryable` is `true`, the error is transient and retrying the same request may succeed. Rate limit exhaustion and provider timeouts are typical examples. When `retryable` is `false`, retrying won't help without a code or configuration change.
