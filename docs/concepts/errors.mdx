...
Better Agent uses `BetterAgentError` as the standard error type across the framework. It carries a stable `code` for programmatic handling so you can branch on specific failure modes without parsing message strings.
