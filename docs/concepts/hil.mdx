...
Approval timeout is checked in order: the return value of `approval.resolve()`, then `approval.timeoutMs` on the tool contract, then `advanced.toolApprovalTimeoutMs` as the app-wide fallback.
