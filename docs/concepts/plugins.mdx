...
Plugins can hook into every phase of the agent loop. The `onStep` hook runs before each step and can mutate messages, set tool choice, filter active tools, or override the system instruction:
...
See the ready-made plugins in the [Plugins](/docs/plugins) section: [Auth](/docs/plugins/auth), Rate Limit, Logging, IP Allowlist, and Sandbox.
