...
Add an `outputSchema` to your agent definition. The `schema` field accepts any Standard Schema library (Zod, Valibot, ArkType) or a raw JSON Schema object.
...
| `strict` | Whether to use strict mode where supported. |
...
The per-run `output` schema fully replaces the agent's default for that run.
