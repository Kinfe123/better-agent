...
Tools let agents do work. Instead of guessing, the model calls your code through a structured contract with validated input. A tool starts as a shared contract, then you materialize it as a server tool, a client tool, or a hosted provider tool.
...
For tools that need human approval before running, see [Human in the Loop](/docs/concepts/hil). For tools sourced from external MCP servers, see [MCP](/docs/concepts/mcp).
